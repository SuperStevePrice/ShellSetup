...
File: ~/.xtrc contains xtrc, colors, and xterm command data.
...
def on_button_click(bg_color, fg_color):
    """
    ...
    def click():
        try:
            xtrc['x_bg'] = bg_color
            xtrc['x_fg'] = fg_color
            print("DEBUG: on_button_click: x_log: ", xtrc['x_log'])
            cmd = xt.set_cmd(xtrc)
...
for i, color_pair in enumerate(colors):
    ...
    button = MacButton(frame_buttons,
        text=f'Xterm Window {str(i + 1).zfill(2)}',
        bg=color_pair[0], fg=color_pair[1],
        activebackground="DarkGrey")
    button.configure(command=on_button_click(color_pair[0], color_pair[1]))
...
button_create_custom_xterm = MacButton(frame_additional,
    text="Create Custom Xterm",
    command=on_button_click("DarkGrey", "NAVY"),
        background="DARKGREY", foreground="NAVY")
